...
func NewHandler(q *pgstore.Queries) http.Handler {
	...
	r.Use(cors.Handler(cors.Options{
		// Remember!!! That the asterisk is so that any site can request the api.
...
func (h apiHandler) handleSubscribe(w http.ResponseWriter, r *http.Request) {
	...
	// Creating a context for the client
	...
	slog.Info("New subscriber connected", "room_id", rawRoomID, "client_id", r.RemoteAddr)
	h.subscribers[rawRoomID][c] = cancel
	h.mu.Unlock()

	<-ctx.Done()

	// Remove the client of the connection's pool
	h.mu.Lock()
